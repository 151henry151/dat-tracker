...
On open, `dat-review` hydrates blank track titles/types from Gemini listen notes (when the model left `tracks` empty and only named songs in notes), reconciles extra cuts down to the published show.txt setlist length when that file lists fewer songs, fills remaining blank **song** titles from that setlist, and seeds empty package fields from the `.txt`, the calibration catalog, and/or the show-id date. Blank banter/intro/tuning/encore tracks get Jon-style titles (`Banter`, `Intro`, …). Seeded package fields then get a **spelling polish** (known fixes such as `Douglass` → `Douglas`, plus optional Gemini text correction when `GEMINI_API_KEY` is set). Your tracker name comes from saved defaults (`~/.config/dat-tracker/review_defaults.json`, or `catalog/operator_defaults.json`, or `$DAT_TRACKER_DEFAULTS`). `set_label` soft-defaults to `One Set`; `transfer` / `transferer` stay per-show (edit in the review form). CLI `--artist` / `--date` / `--venue` / etc. override those seeds.
...
Keys in the TUI: click a yellow cut on the waveform to select it, then `←`/`→` to nudge by 0.1s (`Shift`±1s, `Ctrl`±0.01s); `[`/`]` prev/next cut; `a` Accept-all, `s` Save&approve, `r` Reset to LLM, `q` Quit; Space/`l` play/loop. Insert/delete cut remain on `i`/`d` (hidden from the footer).
