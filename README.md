...
On open, `dat-review` hydrates blank track titles/types from Gemini listen notes (when the model left `tracks` empty and only named songs in notes), reconciles extra cuts down to the published show.txt setlist length when that file lists fewer songs, fills remaining blank **song** titles from that setlist, and seeds empty package fields from companion info text / filenames under `data/calibration` or `data/ground_truth` (Gemini interprets messy layouts when `GEMINI_API_KEY` is set; if venue/city/state are still blank but artist+date are known it may Google-Search for them; heuristic parse is offline fallback only), then catalog/`show_id` date. Blank banter/intro/tuning/encore tracks get Jon-style titles (`Banter`, `Intro`, …). Seeded package fields then get a **spelling polish** (known fixes such as `Douglass` → `Douglas`, plus optional Gemini text correction when `GEMINI_API_KEY` is set). Your tracker name comes from saved defaults (`~/.config/dat-tracker/review_defaults.json`, or `catalog/operator_defaults.json`, or `$DAT_TRACKER_DEFAULTS`). `set_label` soft-defaults to `One Set`; `transfer` / `transferer` stay per-show (edit in the review form). CLI `--artist` / `--date` / `--venue` / etc. override those seeds. Batch-check work shows with `uv run python scripts/verify_work_package_seed.py`.
