#!/usr/bin/env python3
"""Score speech-prioritized cuts against a Tier B synthetic known-cut list."""
...
def main() -> int:
    ...
    parser.add_argument("--energy-min-sep", type=float, default=60.0)
    ...
    if cache.is_file():
        ...
        print(
            f"transcribing {synth} with faster-whisper ({args.model})...",
            file=sys.stderr,
        )
    ...
    naive = cuts_from_speech_segments(segs, duration_sec=duration)
    energy = propose_cuts_from_audio(synth, min_separation_sec=args.energy_min_sep)
    prioritized = propose_speech_prioritized_cuts(
        segs, energy_cuts=energy, duration_sec=duration
    )

    for label, cuts in (
        ("speech_edges_naive", naive),
        ("energy", energy),
        ("speech_prioritized", prioritized),
    ):
        m = summarize_comparison(
            reference_cuts=known,
            hypothesis_cuts=cuts,
        ...
        print(
            ...
            f"cuts={[round(c, 1) for c in cuts]}"
        )
    return 0
